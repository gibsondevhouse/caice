...
final class OllamaSettingsViewModel: ObservableObject {
    ...
    func startOllama() async {
        ...
        do {
            let runtimeState = try await runtimeController.start(endpointURL: endpointURL)
            apply(runtimeState)
            await refreshStatus()
        } catch let error as LocalizedError {
            serviceStatus = .offline
    ...
    func restartOllama() async {
        ...
        do {
            let runtimeState = try await runtimeController.restart(endpointURL: endpointURL)
            apply(runtimeState)
            await refreshStatus()
        } catch let error as LocalizedError {
            serviceStatus = .offline
    ...
    private func fetchModels() async throws -> [ModelInfo] {
        ...
        return decoded.models
            .map { model in
                ...
                return ModelInfo(
                    name: model.name,
                    sizeDescription: ByteCountFormatter.string(fromByteCount: model.size, countStyle: .file),
    ...
    private func apply(_ runtimeState: OllamaRuntimeState) {
        isAppInstalled = runtimeState.isAppInstalled
        isAppRunning = runtimeState.isAppRunning

        if runtimeState.isReachable {
            serviceStatus = .running
            statusDetailText = "Ollama is responding at \(endpointURL.absoluteString)."
        } else if runtimeState.isAppRunning {
            serviceStatus = .offline
            statusDetailText = "Ollama is launching but has not responded yet."
        } else {
            serviceStatus = .offline
